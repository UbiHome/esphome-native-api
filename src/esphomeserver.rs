...
//! ESPHome devices by managing entity keys and state internally. It builds on top of the
...
impl EspHomeServer {
    ...
    /// let stream = TcpStream::connect("192.168.1.100:6053").await?;
    /// let mut server = EspHomeServer::builder().name("client").build();
    /// let (tx, mut rx) = server.start(stream).await?;
    /// # Ok(())
    /// # }
    /// ```
    ...
    /// ```rust
    /// # use esphome_native_api::esphomeserver::{EspHomeServer, Entity, BinarySensor};
    /// let mut server = EspHomeServer::builder().name("server").build();
    /// let sensor = Entity::BinarySensor(BinarySensor {
