...
//! # Protocol Overview
//!
//! The ESPHome native API uses a binary protocol over TCP with the following features:
//! - Protocol buffer message encoding
//! - Optional encryption using Noise protocol framework
...
impl EspHomeApi {
    ...
    /// let stream = TcpStream::connect("192.168.1.100:6053").await?;
    /// let mut api = EspHomeApi::builder().name("client").build();
    /// let (tx, mut rx) = api.start(stream).await?;
    /// # Ok(())
    /// # }
    /// ```
